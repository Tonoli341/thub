...
from app.models import Justification, LdapEmployee
    ...
    <table cellpadding="0" cellspacing="0" border="0" style="display:inline-table;">
      <tr>
        ...
        <td style="vertical-align:middle;">
          <span style="font-family:Arial,Helvetica,sans-serif;font-size:18px;font-weight:700;color:#ffffff;letter-spacing:-0.3px;">T-Hub</span>
        </td>
      </tr>
    </table>"""
...
<title>{title}</title>
<style>
  body, table, td {{ margin:0; padding:0; border:0; font-family:Arial,Helvetica,sans-serif; }}
      ...
      <table width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;">
        ...
        <tr>
          <td style="background:#007040;border-radius:12px 12px 0 0;padding:20px 32px;">
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td>{logo}</td>
                <td align="right" style="font-family:Arial,Helvetica,sans-serif;font-size:11px;color:rgba(255,255,255,0.65);letter-spacing:0.05em;text-transform:uppercase;">Workforce Planner</td>
              </tr>
            </table>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#F0ECE0;border-radius:0 0 12px 12px;padding:16px 32px;border:1px solid #e0dcd4;border-top:none;">
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td style="font-family:Arial,Helvetica,sans-serif;font-size:11px;color:#2B2B2B;opacity:0.55;">
...
def _info_row(label: str, value: str) -> str:
    return f"""
    <tr>
      <td style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#6c757d;padding:6px 0 0;width:130px;vertical-align:top;">{label}</td>
      <td style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#2B2B2B;font-weight:600;padding:6px 0 0;vertical-align:top;">{value}</td>
    </tr>"""
...
def notify_approvers_new_request(db: Session, justification: Justification) -> None:
    ...
    emails = list({get_employee_email(db, eid) for eid in approver_ids if eid} - {None})
    if not emails:
        return
    ...
    text = (
        ...
        f"Tipo: {type_label}\n"
        f"Periodo: {period}\n\n"
        f"Accedi al portale T-Hub per gestire la richiesta.\n\n"
        f"— T-Hub Workforce Planner · Tonoli S.p.A."
    )

    body_html = f"""
    <p style="font-family:Arial,Helvetica,sans-serif;font-size:15px;font-weight:700;color:#2B2B2B;margin:0 0 6px;">
      Nuova richiesta di {type_label.lower()}
    </p>
    <p style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#555;margin:0 0 24px;line-height:1.5;">
    ...
    <p style="font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#6c757d;margin:20px 0 0;line-height:1.5;">
      Accedi al portale T-Hub per approvare o rifiutare la richiesta.
    </p>
    """

    _send(emails, subject, text, _wrap_html(subject, body_html, f"Richiesta di {type_label.lower()} da {employee_name} · {period}"))
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
      ...
      <tr><td style="padding:14px 20px;">
        <table cellpadding="0" cellspacing="0" border="0">
          <tr>
            <td style="font-family:Arial,Helvetica,sans-serif;font-size:20px;font-weight:800;color:{status_color};padding-right:12px;">
              {"✓" if status == "approved" else "✕"}
            </td>
            <td>
              <p style="font-family:Arial,Helvetica,sans-serif;font-size:15px;font-weight:700;color:{status_color};margin:0;">
                Richiesta {status_label.lower()}
              </p>
              <p style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:{status_color};opacity:0.8;margin:2px 0 0;">
