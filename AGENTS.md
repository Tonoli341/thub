...
| Tabella `ninjaone_tickets` | [migrations/versions/0010_ninjaone_tickets.py](backend/migrations/versions/0010_ninjaone_tickets.py) | Il feature "apertura ticket NinjaOne da T-Hub" è stato revertato dal codice (commit `4e6bb9b`, 2026-09-01) dopo che questa migrazione era già applicata in produzione con `down_revision=0003` (vedi commit `045fcc7`): il file è stato ricreato il 2026-09-03 solo per continuità della catena Alembic, riagganciato allo stesso punto reale (`0003_assignment_cause_visita_idoneita_lowercase`), con la catena Manutenzioni (`0004_maintenance_assets`, id di revisione `0003_maintenance_assets`) spostata a valle di questo nodo invece che direttamente su 0003. Nessun modello/router/pagina usa più questa tabella — non reintrodurre il feature senza deciderlo esplicitamente |
